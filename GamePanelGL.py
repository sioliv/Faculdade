    ...
    def run(self, value=1):
        ...
        if (abs(currentTime - self.previousTimeStamp) >= 1000):
            if GameMain.FPS and value:
                print("fps = %d, delay = %d ms \r" % (self.numberOfFramesForFPS, self.speed), end='')
                sys.stdout.flush()
            self.numberOfFramesForFPS = 0
...
class GamePanel:
    ...
    def __init__(self, game, windowID, scorePanel=None):
        ...
        self.timer = Timer(self.fallingSpeed, glutPostRedisplay)  # self.TimerCallback
        self.timer.run()
    ...
    def unProject(self, x, y):
        ...
        wcx, wcy, wcz = gluUnProject(x, y, 0, modelMatrix, projMatrix, viewport)
    ...
    def paintComponent(self):
        ...
        if self.currentCell is not None:
            self.highlightOneCell(self.currentCell.row(), self.currentCell.col())
    ...
    def highlightOneCell(self, row, col):
        self.setColor(Color.WHITE)
        self.setStroke(2)
        self.drawRect(col * GameMain.SIZE, row * GameMain.SIZE, GameMain.SIZE, GameMain.SIZE)
        self.setStroke(1)
    ...
    def TimerCallback(self):
        ...
        if (self.flashingState == 0 and not self.collapsing and not self.filling):
            ...
            if len(self.cellsToCollapse) != 0:
                ...
                self.scorePanel.updateScore(self.game.getScore(), not self.game.debug and self.scorePanel.g is None)
            else:
        ...
        if self.collapsing:
            ...
            if not found:
                ...
                if len(self.fillingCells) == 0:
                    print("WARNING: game returned collapsing cells but failed to return cells to fill columns")
                    self.filling = False
                    self.fillingCells = None
...
class AnimatedCell(Cell):
    ...
    def __init__(self, cell, startRow=None):
        super(AnimatedCell, self).__init__(cell.row(), cell.col(), cell.getIcon())

        if startRow is not None:
            ## This cell previous row.
            self.previousRow = startRow
            ## Starting cell position in pixels.
            self.startPixel = startRow * GameMain.SIZE  # row * (pixels in a cell)
...
class GameMain:
    ...
    # - Topaz, Emerald, Garnet, Ruby, Diamond, Amethyst and Sapphire (the main icon of all Bejeweled games).
    ...
    def create(self, argv, npanels=0):
        ...
        game = GameImpl(GameMain.GRID_SIZE[0], GameMain.GRID_SIZE[1],
                        BasicGenerator(GameMain.ICON_TYPES, GameMain.SEED))
        ...
        glutInitWindowSize(2 * width if threePanels or onePanel else width, height)
        ...
        if threePanels:
            ...
            self.gameWindow = glutCreateSubWindow(self.mainWindow, width, 0, width, height)
            glutKeyboardFunc(self.keyPressed)
            ## score panel window ID.
            self.scoreWindow = glutCreateSubWindow(self.mainWindow, 0, 0, width, height)
            glutKeyboardFunc(self.keyPressed)
        elif twoPanels:
            ## score panel window ID.
        ...
        try:
            try:
                ...
                opts, args = getopt.getopt(argv[1:], "ht:c:r:s:d:v:p:",
                                           ["help", "types=", "width=", "height=", "speed=", "seed=", "verbose=",
                                            "panel="])
            ...
            for opt, arg in opts:  # something such as [('-h', '')] or [('--help', '')]
                if opt in ("-h", "--help"):
                    print(
                        "Usage %s -t <icon_types> -c <grid_width> -r <grid_height> -s <speed> -d <seed> -v <level> -p <score_window>" %
                        argv[0])
                    return 1
                elif opt in ("-c", "--width"):
