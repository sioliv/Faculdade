...
class GamePanel:
    ...
    def fillRect(self, x, y, w, h):
        self.g.create_rectangle(x, y, x + w, y + h, outline='black', fill=self.color)

    ## Draw a rectangle.
    def drawRect(self, x, y, w, h):
        self.g.create_rectangle(x, y, x + w, y + h, outline=self.color, width=self.stroke)
    ...
    def paintComponent(self, g):
        ...
        self.fillRect(0, 0, self.game.getWidth() * GameMain.GRID_SIZE, self.game.getHeight() * GameMain.GRID_SIZE)
        ...
        if self.currentCell is not None:
            self.highlightOneCell(g, self.currentCell.row(), self.currentCell.col())
...
class makeThread(Thread):
    ...
    #
    def stop(self):
        if (self.debug): print("Thread stop")
        self.__on = False
...
class ScorePanel:
    ...
    def paintComponent(self):
        ...
        temp = self.g.create_text(0, 0, font=font, anchor=NW, text=txt, tag="t1")
        b = self.g.bbox(temp)
        # size of temp.
        width = b[2] - b[0]
        height = b[3] - b[1]
...
class GameMain:
    ...
    def create(self):
        ...
        game = GameImpl(GameMain.GRID_SIZE, GameMain.GRID_SIZE, BasicGenerator(GameMain.ICON_TYPES))
        if (GameMain.__DEBUG__): game.debug = True

        # arrange the two panels horizontally
        mainPanel = Tk()
        canvas = Canvas(mainPanel, width=game.getWidth() * GameMain.SIZE, height=game.getHeight() * GameMain.SIZE,
                        background='dark grey')
        canvas.pack(side=RIGHT, fill=BOTH, expand=YES)
        canvas2 = Canvas(mainPanel, width=game.getWidth() * GameMain.SIZE, height=game.getHeight() * GameMain.SIZE,
                         background='white')
    ...
    def main(self, args=None):
        ...
        if (len(args) > 1): GameMain.__DEBUG__ = True
